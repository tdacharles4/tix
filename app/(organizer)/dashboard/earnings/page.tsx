    ...
    if (!profile?.stripe_onboarding_complete || !profile.stripe_account_id) {
        return (
            <div className="max-w-2xl mx-auto px-4 py-16 text-center">
                <h1 className="text-2xl font-bold text-gray-900 mb-3">Mis ingresos</h1>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="border border-gray-200 rounded-xl p-5">
                    ...
                    <p className="text-2xl font-bold text-gray-900">{formatMXN(availableCents / 100)}</p>
                    <p className="text-xs text-gray-400 mt-1">Listo para depositar</p>
                </div>
                <div className="border border-gray-200 rounded-xl p-5">
                    <p className="text-xs text-gray-500 mb-1">En tránsito</p>
                    <p className="text-2xl font-bold text-gray-900">{formatMXN(pendingCents / 100)}</p>
            ...
            {/* Recent charges */}
            <div className="mb-8">
                <h2 className="text-sm font-semibold text-gray-700 mb-3">Cobros recientes</h2>
                {!transactions?.data.length ? (
                    <p className="text-sm text-gray-400 py-6 text-center border border-gray-100 rounded-lg">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-gray-200 bg-gray-50">
                                    ...
                                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-gray-500">Bruto</th>
                                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-gray-500">Comisiones</th>
                                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-gray-500">Neto</th>
                                </tr>
                            </thead>
                            <tbody>
            ...
            {/* Payouts */}
            <div>
                <h2 className="text-sm font-semibold text-gray-700 mb-3">Depósitos a tu cuenta</h2>
                {!payouts?.data.length ? (
                    <p className="text-sm text-gray-400 py-6 text-center border border-gray-100 rounded-lg">
                    ...
                    <div className="border border-gray-200 rounded-lg overflow-hidden">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-gray-200 bg-gray-50">
                                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500">Fecha de llegada</th>
                                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-gray-500">Monto</th>
                                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 pl-6">Estado</th>
                                </tr>
                            </thead>
                            <tbody>
                                {payouts.data.map((payout) => (
                                    <tr key={payout.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 pl-6">
                                            <span className={`px-2 py-1 rounded text-xs font-medium ${payoutStatusColor[payout.status] ?? 'bg-gray-100 text-gray-500'}`}>
